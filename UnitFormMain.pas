...
uses
  ...
  Dialogs, ExtCtrls, StrUtils, StdCtrls, Winapi.Richedit,
...
type
  ...
  TFormMain = class(TForm)
    ...
    procedure FrameConfigLine1TimerCheckTimer(Sender: TObject);
  private
...
procedure TFormMain.ChangeIconLogo;
var
  Image: TPngImage;
begin
  Image := TPngImage.Create;
  try
    Image.LoadFromResourceName(Hinstance, 'LOGO');
...
begin
  ...
  LoadSettings;
  ChangeIconLogo;
  LabelCopyRight.Caption := 'VestaRf http://vesta.prodomos.ru <prodomos.ru@gmail.com>';
end;
...
procedure TFormMain.FrameConfigLine1TimerCheckTimer(Sender: TObject);
begin
  FrameConfigLine1.TimerCheckTimer(Sender);
...
begin
  TStringChest[FrameConfigLine1.Name + '.FilePath'] := 'INTERNAL://VESTA_RF';
  ...
  TStringChest.Publisher := 'VestaRF';
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        MemoOutput.Lines.Add('Serial port connected.');
        BurnOK := False;
        RunState := StateHandshake;
        TimerStateMachine.Enabled := True;
        ChangeIconWait;
        MemoOutput.Lines.Add('Begin find ESP8266.');
  ...
  begin
    ...
    if (not(CommMain.PortOpen)) then
    begin
      MemoOutput.Lines.Add('Serial port disconnected.');
      BurnOK := True;
      RunState := StateFinished;
      TimerStateMachine.Enabled := False;
      if (Sender = ActionBurn) then
      begin
        MemoOutput.Lines.Add('Serial port closed by user.');
        ChangeIconFail;
      end;
      // Set RTS to HIGH
      // CommMain.RtsControl := TRtsControl.RtsDisable;
...
//    case TextIndex of
//      0:
//        LabelIntroduction.Caption := 'VestaRf';
//      1:
//        LabelIntroduction.Caption := '';
//      2:
//        LabelIntroduction.Caption := 'gateway';
//      3:
//        LabelIntroduction.Caption := '';
//      4:
//        LabelIntroduction.Caption := 'for project';
//      5:
//        LabelIntroduction.Caption := '';
//      6:
//        LabelIntroduction.Caption := 'MySensors';
//      7:
//        LabelIntroduction.Caption := '';
//    else
//      TextIndex := 0;
...
begin
  ...
  begin
    ...
    MemoOutput.Lines.Add('Detect serial port changed.');
    if (SerialPorts.Count >= 1) then
    begin
      ComboBoxSerialPortA.ItemIndex := ComboBoxSerialPortA.Items.Count - 1;
      MemoOutput.Lines.Add('Auto MAP serial port.' + 'Port-->' +
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        MemoOutput.Lines.Add('ESP8266 ACK success.');
        RunState := StateBurn;
      end;
      if (Pos(LeftStr(ESP_SET_BASE_ADDRESS_ACK, 5), SerailBufferA) <> 0) then
      begin
        MemoOutput.Lines.Add('Set base address success.');
        MemoOutput.Lines.Add('Loading firmware: ');
        LastOperationSuccess := True;
      end;
      if (Pos(LeftStr(ESP_SEND_DATA_ACK, 5), SerailBufferA) <> 0) then
      begin
        MemoOutput.SelStart:=32000; // ×òîáû êóðñîð áûë âñåãäà ïîñëå ïîñëåäíåãî
        MemoOutput.SelText:='.';
        LastOperationSuccess := True;
        //MemoOutput.Text:=MemoOutput.Text+'.';
        // MemoOutput.Perform(WM_CHAR, Ord('*'), 0);
        //MemoOutput.SelText:='.';
      end;
      if (Pos(LeftStr(ESP_RUN_ACK, 5), SerailBufferA) <> 0) then
      begin
        MemoOutput.Lines.Add('Successfully!');
        RunState := StateFinished;
        ChangeIconSuccess;
        if (CommMain.PortOpen) then
